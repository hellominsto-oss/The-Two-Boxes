...
export default function App() {
  ...
  const assertSupply = (players: Player[], expectedSupply: number) => {
    if (
      !coinSupplyIsConserved(players, expectedSupply) ||
      totalCoins(players) !== expectedSupply
    ) {
      throw new Error(
        `Coin supply invariant failed: expected ${expectedSupply}, got ${totalCoins(players)}`,
      );
    }
  };
  ...
  const submitContribution = (c: Contribution) => {
    ...
    if (nextIndex < state.queue.length) {
      ...
      const { outcomes, minMet } = resolveRound(
        state.players,
        state.minimum,
        contributions,
      );
      ...
      setState((s) => ({
        ...
        queueIndex: 0,
        handoff: ids.length
          ? { playerId: ids[0], next: "privateStatement" }
          : null,
  ...
  const cancelAddPlayer = () =>
    setState((s) => ({
      ...s,
      phase:
        s.phase === "addPlayer" && activePlayers(s.players).length === 2
          ? "twoPlayerDecision"
          : "roundEndDecision",
    }));
    ...
    const prompt =
      next === "privateTurn"
        ...
        : next === "privateStatement"
          ? t.handoffStatement
          : t.handoffAccusation;
    const onReady = () => setState((s) => ({ ...s, phase: next }));
